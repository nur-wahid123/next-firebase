...
const Home = () => {
  ...
  useEffect(() => {
    if (alert) {
      // Only set timeout if alert is not empty
      setFormData({ ...formData, display: 1 });
      const timeoutId = setTimeout(() => {
        setAlert("");
        setFormData({ ...formData, display: 0 });
      }, 5000);

      return () => clearTimeout(timeoutId); // Clear the timeout if the component unmounts or alert changes
    }
  }, [alert]);
  return (
    <div>
